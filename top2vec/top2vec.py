...
class Top2Vec:
    ...
    def compute_topics(self,
        ...
        contextual_top2vec: bool (default None)
            If True, the model will be able to assign topics at token level.
            Initial model must have been trained with contextual_top2vec=True.

        c_top2vec_smoothing_window: int (default 5)
            The size of the window used to smooth the document token embeddings.
        """
        ...
        if self.contextual_top2vec & contextual_top2vec:
            ...
            self.doc_top_tokens = doc_top_tokens
            self.doc_topic_distribution = doc_topic_distribution
            self.doc_topic_scores = doc_topic_scores
            self.doc_top_token_dists = doc_top_token_dists
            self.topic_sizes = topic_sizes

            # TODO: create function to return contextual_top2vec output

        elif contextual_top2vec & (not self.contextual_top2vec):
            raise ValueError("Initial model must have been trained with contextual_top2vec=True.")
    ...
    @contextual_top2vec_req(True)
    def get_document_topic_distribution(self) -> np.ndarray:

        """
        Returns the document topic distribution.

        Returns
        -------
        doc_topic_distribution: numpy.ndarray
            An array of shape (num_documents, num_topics) where each row
            represents the probability distribution of topics for a document.
        """

        return self.doc_topic_distribution

    @contextual_top2vec_req(True)
    def get_document_topic_relevance(self) -> np.ndarray:

        """
        Returns the document topic relevance.

        Returns
        -------
        doc_topic_scores: numpy.ndarray
            An array of shape (num_documents, num_topics) where each row
            represents the relevance of topics for a document.
        """

        return self.doc_topic_scores

    @staticmethod
    def _create_document_topic_structure(data, scores):
        documents = []

        for doc_idx, topics_dict in data.items():
            # Fetch the document scores for the current document index
            doc_scores = scores.get(doc_idx, {})

            # Create a list of Topic namedtuples, each with tokens and score
            topics = [
                Topic(topic_index=topic_idx, tokens=tokens, score=doc_scores.get(topic_idx, []))
                for topic_idx, tokens in topics_dict.items()
            ]

            # Create a Document namedtuple for each document
            documents.append(Document(document_index=doc_idx, topics=topics))

        return documents

    @contextual_top2vec_req(True)
    def get_document_token_topic_assignment(self) -> List[Document]:
        """
        Returns the token-level topic assignments for each document.

        Returns
        -------
        documents: list
            A list of Document, each containing a list of Topic with token assignments
            and scores for each token.
        """

        return self._create_document_topic_structure(self.doc_top_tokens, self.doc_top_token_dists)
    ...
    def get_document_tokens(self):
        ...
        Returns
        -------
        document_tokens: list of list[str]
            Were each list of strings represents the
    ...
    def get_topic_sizes(self, reduced=False):
        ...
        Top2vec: the number of documents most similar to each topic.
        Topics are in increasing order of size.
    ...
    def hierarchical_topic_reduction(self, num_topics, interval=None):
        ...
        # progress
        pbar = tqdm(total=num_topics_current - num_topics, desc="Reducing topics")
        ...
        if self.contextual_top2vec:
            ...
            self.doc_topic_distribution_reduced = doc_topic_distribution
            self.doc_topic_scores_reduced = doc_topic_scores
            self.doc_top_token_dists_reduced = doc_top_token_dists
            self.topic_sizes_reduced = topic_sizes
